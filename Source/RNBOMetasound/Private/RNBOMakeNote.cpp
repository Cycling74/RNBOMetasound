...
class FMakeNoteOperator : public TExecutableOperator<FMakeNoteOperator>
{
  public:
    ...
    void Execute()
    {
        ...
        if (num > 0) {
            const auto note = static_cast<int32>(std::clamp(*NoteNum, 0, 127));
            const auto vel = static_cast<int32>(std::clamp(*NoteVel, 1, 127));
            //const auto offvel = static_cast<int32>(std::clamp(*NoteOffVel, 0, 127));
            const auto chan = static_cast<int32>(std::clamp(*NoteChan, 0, 15));
            ...
            for (auto i = 0; i < num; i++) {
                ...
                HarmonixMetasound::FMidiStreamEvent noteoff(id, FMidiMsg::CreateNoteOff(chan, note));
